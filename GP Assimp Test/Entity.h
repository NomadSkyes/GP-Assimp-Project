...
public:
	...
	void MoveToward(glm::vec3 _target)
	{
		/*float angle = atan2((this->position.x - _target.x), (this->position.z - _target.z)) * (180 / PI);*/
		glm::vec3 initialVector(0,0,0);

		float angle = glm::radians(this->getAngle());

		initialVector.x = sin(angle);
		initialVector.z = cos(angle);
		//initialVector.x = (this->position.x - _target.x);
		//initialVector.z = (this->position.z - _target.z);
		initialVector = glm::normalize(initialVector);
		glm::vec3 movementVector = initialVector * this->movementSpeed;
		SetPosition(-movementVector);
	}
	...
	void Kill() {
		cout << "dead" << endl;
		this->isAlive = false;
		if (this->_multiRb.size() > 0)
		{
			for (int i = 0; i < this->_multiRb.size(); i++)
			{
				_cs->getWorld()->removeRigidBody(this->_multiRb[i]);
			}
		}
	...
	{
		float returnAngle = atan2((this->position.x - target.x), (this->position.z - target.z));
		SetAngle(returnAngle * (180 / PI));
